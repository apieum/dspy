...
class TestSimplifiedReflectiveMutation:
    ...
    def test_minibatch_sampling(self):
        ...
        feedback_provider = FeedbackProvider(metric=lambda ex, pred: 0.5)
        generator = ReflectivePromptMutation(feedback_provider, minibatch_size=2)
        ...
        dataset_manager = DefaultDatasetManager(feedback_data)
        generator.dataset_manager = dataset_manager
        
        # Test sampling
        minibatch = generator.dataset_manager.get_feedback_minibatch(2)
        assert len(minibatch) == 2  # Respects minibatch_size
        assert all(ex in feedback_data for ex in minibatch)  # Samples from feedback_data
        
        # Test with limited data
        limited_data = feedback_data[:1]
        limited_dataset_manager = DefaultDatasetManager(limited_data)
        generator.dataset_manager = limited_dataset_manager
        minibatch = generator.dataset_manager.get_feedback_minibatch(2)
