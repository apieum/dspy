...
def test_gepa_delegates_configuration():
    ...
    def mock_next_generation(parents):
        ...
        if not configuration_checked:
            ...
            assert dataset_mgr.num_pareto_tasks >= 1
            assert dataset_mgr.num_feedback_examples >= 1
            assert dataset_mgr.num_pareto_tasks + dataset_mgr.num_feedback_examples == len(training_data)
...
def test_dataset_split_algorithm():
    ...
    def mock_next_generation(parents):
        ...
        if not split_captured:
            ...
            dataset_mgr = gepa.evaluator.dataset_manager
            pareto_size = dataset_mgr.num_pareto_tasks
            feedback_size = dataset_mgr.num_feedback_examples
            
            # Should preserve total dataset size
            assert pareto_size + feedback_size == len(training_data)
            
            # Should have reasonable split (not all to one dataset)
            assert pareto_size >= 1
            assert feedback_size >= 1
