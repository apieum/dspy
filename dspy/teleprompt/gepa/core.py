...
class GEPA:
    ...
    def compile(self, student: Module,
        ...
        gen_0 = Generation([initial_candidate], generation_id=0, iteration=0)
    ...
    def _show_results(self, evaluation_data: List[dspy.Example]) -> Module:
        ...
        # Use balanced top filtering to get the best candidate
        from .filtering import BalancedTop
        filtering = BalancedTop(keep_top_n=1)
        best_candidates = self.candidate_pool.filter_top(self.candidate_pool.size(), filtering)
    ...
    def create_basic(metric, max_calls: int = 1000, population_size: int = 10) -> "GEPA":
        ...
        from .selection import ElitistSelection
        ...
        return GEPA(
            ...
            selection=ElitistSelection(keep_top_n=5),
    ...
    @staticmethod
    def create_diversity(metric, max_calls: int = 1500, population_size: int = 15) -> "GEPA":
    ...
    def create_iteration_limited(metric, max_iterations: int = 20, population_size: int = 8) -> "GEPA":
        ...
        return GEPA(
            ...
            scoring=ParetoScoring(metric),
            selection=ElitistSelection(keep_top_n=4),
            generator=MutationGenerator(
                mutation_rate=0.4,
                population_size=population_size
            ),
